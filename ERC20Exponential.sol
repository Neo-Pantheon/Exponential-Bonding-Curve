...
contract ERC20Exponential is ERC20, Ownable {
    uint256 public constant INITIAL_PRICE = 0.000001 ether; // Starting price
    uint256 public constant K = 1000; // Exponential steepness factor
    ...
    constructor(string memory name, string memory symbol) ERC20(name, symbol) Ownable(msg.sender) {
        // Ownable constructor requires the initial owner to be passed (msg.sender is the deployer)
        transferOwnership(msg.sender); // Set the deployer as the initial owner
    }

    // Calculate token price based on current supply using exponential formula
    function calculateCurrentPrice() public view returns (uint256) {
        uint256 supply = totalSupply();
        // Price increases exponentially with supply
        return INITIAL_PRICE * (10**18 + (K * supply)) / 10**18;
    }
